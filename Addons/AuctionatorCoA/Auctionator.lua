...
local auctionator_savedvars_defaults =
	...
	["BARGAIN_ALERTS"]		= 1;	-- alert after a full scan
	["BARGAIN_DISCOUNT"]	= 50;	-- minimum percentage below the reference price
	["BARGAIN_MIN_PROFIT"]	= 50000;	-- 5 gold after the auction-house cut
	["BARGAIN_MAX_RESULTS"]	= 20;
	["BARGAIN_AH_CUT"]		= 5;	-- standard auction-house commission percentage
	["BARGAIN_QUICK_ITEMS"]	= 200;	-- retained for compatibility with older saved settings
	["INVENTORY_MIN_VALUE"]	= 10000;	-- one gold for the complete quantity owned
	};
...
local function Atr_GetAuctionatorMemString(msg)
	...
	local mem  = GetAddOnMemoryUsage("AuctionatorCoA");
...
local function Atr_SlashCmdFunction(msg)
	...
	elseif (cmd == "deals" and Atr_HandleBargainCommand) then
		Atr_HandleBargainCommand (param1, param2);
...
function Atr_OnLoad()
	...
	AuctionatorVersion = GetAddOnMetadata("AuctionatorCoA", "Version");
...
--	zc.msg("Auctionator Initialized");
	...
	-- Add defaults introduced by newer versions without resetting existing
	-- Auctionator preferences.
	for name,value in pairs (auctionator_savedvars_defaults) do
		if (AUCTIONATOR_SAVEDVARS[name] == nil) then
			AUCTIONATOR_SAVEDVARS[name] = value;
		end
	end
	...
	gCommandsPane = Atr_AddSellTab ("Commandes",		COMMANDS_TAB);
...
local _AUCTIONATOR_COMMANDS_TAB_INDEX = 0;
...
function Atr_FindTabIndex (whichTab)
	...
	if (_AUCTIONATOR_SELL_TAB_INDEX == 0) then
		...
		while (true)  do
			...
			if (tab.auctionatorTab) then
				...
				if (tab.auctionatorTab == COMMANDS_TAB)	then _AUCTIONATOR_COMMANDS_TAB_INDEX = i; end;
			end
	...
	if (whichTab == COMMANDS_TAB) then return _AUCTIONATOR_COMMANDS_TAB_INDEX; end;
...
function Atr_UpdateCommandsHelp ()

	if (not Atr_CommandsHTML) then
		return;
	end

	local discount = AUCTIONATOR_SAVEDVARS.BARGAIN_DISCOUNT or 50;
	local profit = AUCTIONATOR_SAVEDVARS.BARGAIN_MIN_PROFIT or 50000;
	local cut = AUCTIONATOR_SAVEDVARS.BARGAIN_AH_CUT or 5;
	local watchItems = 0;
	if (Atr_GetWatchlistCount) then
		watchItems = Atr_GetWatchlistCount();
	elseif (type(AUCTIONATOR_SAVEDVARS.BARGAIN_WATCHLIST) == "table") then
	...
	local helpText = "<html><body>"
		...
		.."|cff00ff00Scan surveillance|r : verifie seulement ta liste cible ("..watchItems.." objets) et apprend leurs prix.</p>"
		.."<h1>|cffffd100Liste de surveillance|r</h1>"
		.."<p>|cffffffff/atr watch|r - affiche la liste actuelle.<br/>"
		.."|cffffffff/atr watch add Silk Cloth|r - ajoute un objet.<br/>"
		.."|cffffffff/atr watch remove Silk Cloth|r - retire un objet.<br/>"
		.."|cffffffff/atr watch scan|r - lance le scan cible.<br/>"
		.."|cffffffff/atr watch reset|r - restaure la liste conseillee.<br/>"
		.."Trois scans suffisent pour commencer la comparaison historique.</p>"
		.."<h1>|cffffd100Alertes bonnes affaires|r</h1>"
		.."<p>|cffffffff/atr deals|r - affiche les reglages actuels.<br/>"
		.."|cffffffff/atr deals on|off|r - active ou coupe les alertes.<br/>"
		.."|cffffffff/atr deals discount 35|r - reduction minimale en pourcentage.<br/>"
		.."|cffffffff/atr deals profit 2|r - profit net minimal en pieces d'or.<br/>"
		...
		.."|cffffffff/atr value min 5|r - n'affiche que les piles valant au moins 5 pieces d'or.<br/>"
		.."La banque personnelle et la banque de guilde sont memorisees quand tu les ouvres.</p>"
		.."<h1>|cffffd100Ventes et entretien|r</h1>"
		.."<p>|cff00ff00Check for Undercuts|r - verifie si tes ventes ont ete sous-cotees.<br/>"
		.."|cffffffff/atr mem|r - affiche la memoire utilisee.<br/>"
		.."|cffff4040/atr clear fullscandb|r - efface toute la base de scan.<br/>"
		.."|cffff4040/atr clear posthistory|r - efface l'historique de tes ventes.</p>"
		.."</body></html>";

	Atr_CommandsHTML:SetText (helpText);
	Atr_CommandsHTML:SetSpacing (2);
end
...
function Atr_AuctionFrameTab_OnClick (self, index, down)
	...
	elseif (Atr_IsAuctionatorTab(index)) then
		...
		if (index ~= Atr_FindTabIndex(COMMANDS_TAB)) then
			Atr_HeadingsBar:Show();
			AuctionatorScrollFrame:Show();
			Atr_CancelSelectionButton:Show();
		end

		if (index == Atr_FindTabIndex(SELL_TAB)) then
			Atr_SellControls:Show();
		elseif (index == Atr_FindTabIndex(COMMANDS_TAB)) then
			Atr_HeadingsBar:Hide();
			AuctionatorScrollFrame:Hide();
			Atr_ListTabs:Hide();
		...
		if (gOpenAllBags == 1 and index ~= Atr_FindTabIndex(COMMANDS_TAB)) then
			OpenAllBags(true);
			gOpenAllBags = 0;
		end
...
function Atr_OnAuctionUpdate ()
	...
	if (gAtr_FullScanState == ATR_FS_STARTED) then
		...
		-- A single malformed auction entry must not be able to wedge the whole
		-- addon: without this, an error here leaves gAtr_FullScanState stuck at
		-- ATR_FS_STARTED forever, and every future auction-list update -- not
		-- just the scan's -- gets silently swallowed by this same branch.

		local ok, err = pcall (Atr_FullScanAnalyze);

		if (not ok) then
			zc.msg_red ("Auctionator: "..ZT("Scan interrupted").." (erreur: "..tostring(err)..")");
			if (Atr_FullScanAbort) then
				Atr_FullScanAbort();
			else
...
function Atr_IsTabSelected(whichTab)
	...
	if (not whichTab) then
		return (Atr_IsTabSelected(SELL_TAB) or Atr_IsTabSelected(MORE_TAB) or Atr_IsTabSelected(BUY_TAB) or Atr_IsTabSelected(COMMANDS_TAB));
	end
...
function Atr_IsAuctionatorTab (tabIndex)
	...
	if (tabIndex == Atr_FindTabIndex(SELL_TAB) or tabIndex == Atr_FindTabIndex(MORE_TAB) or tabIndex == Atr_FindTabIndex(BUY_TAB) or tabIndex == Atr_FindTabIndex(COMMANDS_TAB) ) then
